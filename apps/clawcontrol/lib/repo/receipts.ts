...
import { redactSecrets } from "../security/redact-output";
...
export function createDbReceiptsRepo(): ReceiptsRepo {
  return {
    ...
    async append(
      ...
      const field =
        input.stream === "stdout" ? "stdoutExcerpt" : "stderrExcerpt";
      const currentValue = existing[field];
      const sanitizedChunk = redactSecrets(input.chunk);
      const newValue = currentValue + sanitizedChunk;
      ...
      publishReceiptChunk(id, input.stream, sanitizedChunk);
