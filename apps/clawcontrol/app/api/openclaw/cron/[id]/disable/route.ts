...
export async function POST(
  ...
  try {
    const result = await runDynamicCommand('cron.disable', { id: jobId }, {
    ...
    const parsed = parseJsonFromCommandOutput<DisableResult>(result.stdout)

    return NextResponse.json({
      status: latencyMs > DEGRADED_THRESHOLD_MS ? 'degraded' : 'ok',
      latencyMs,
      data: parsed ?? { jobId, enabled: false },
      error: null,
      timestamp: new Date().toISOString(),
      cached: false,
    })
