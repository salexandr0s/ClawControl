...
function isBrokenPipeError(error: unknown): error is NodeJS.ErrnoException {
  ...
  return (
    maybeErrno.code === "EPIPE" || maybeErrno.code === "ERR_STREAM_DESTROYED"
  );
}
...
function readDesktopUpdateState(): DesktopUpdateState {
  ...
  try {
    ...
    return {
      ...
      raw.lastSeenVersion.trim().length > 0
...
async function getDesktopUpdateInfo(): Promise<DesktopUpdateInfo> {
  ...
  return {
    ...
    releaseName:
      ...
      latestRelease.name.trim().length > 0
...
async function waitForServerStop(
  timeoutMs: number = SERVER_STOP_TIMEOUT_MS,
): Promise<boolean> {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
...
function runModelAuthLoginInTerminal(
  providerId: string,
): RunModelAuthLoginResponse {
  ...
  if (process.platform === "win32") {
    ...
    const result = launchDetached("cmd.exe", [
      "/c",
      "start",
      "",
      "cmd",
  ...
  const linuxTerminalCandidates: Array<{ bin: string; args: string[] }> = [
    {
      ...
      args: ["-e", "bash", "-lc", `${posixCommand}; exec bash`],
    },
    {
      bin: "gnome-terminal",
      args: ["--", "bash", "-lc", `${posixCommand}; exec bash`],
    },
    {
      bin: "konsole",
      args: ["-e", "bash", "-lc", `${posixCommand}; exec bash`],
...
// ---------------------------------------------------------------------------
// Gateway token encryption via Electron safeStorage (CC-002)
// ---------------------------------------------------------------------------
...
function decryptToken(base64: string): string | null {
  if (!safeStorage.isEncryptionAvailable()) return null;
  try {
...
// ---------------------------------------------------------------------------
// Per-install auth secret (mirrors secret-store.ts for the Electron side)
// ---------------------------------------------------------------------------
...
function ensureAuthSecret(): string {
  ...
  if (fromEnv && fromEnv.length >= AUTH_SECRET_MIN_LENGTH) return fromEnv;

  const dataDir = app.getPath("userData");
  const secretPath = path.join(dataDir, AUTH_SECRET_FILE_NAME);

  try {
    const content = fs.readFileSync(secretPath, "utf8").trim();
    if (content.length >= AUTH_SECRET_MIN_LENGTH) return content;
  } catch {
...
function readDesktopSettings(): DesktopSettings {
  ...
  try {
    ...
    const settings: DesktopSettings = {
      ...(normalizeSettingsString(raw.gatewayHttpUrl)
        ? {
            gatewayHttpUrl:
              normalizeSettingsString(raw.gatewayHttpUrl) ?? undefined,
          }
        : {}),
      ...(normalizeSettingsString(raw.gatewayWsUrl)
        ? {
            gatewayWsUrl:
              normalizeSettingsString(raw.gatewayWsUrl) ?? undefined,
          }
        : {}),
      ...(normalizeSettingsString(raw.workspacePath)
        ? {
            workspacePath:
              normalizeSettingsString(raw.workspacePath) ?? undefined,
          }
        : {}),
      ...(typeof raw.setupCompleted === "boolean"
        ? { setupCompleted: raw.setupCompleted }
        : {}),
      ...(normalizeSettingsString(raw.updatedAt)
        ? { updatedAt: normalizeSettingsString(raw.updatedAt) ?? undefined }
        : {}),
    };

    // Decrypt gateway token if stored encrypted; migrate plaintext to encrypted
    const encryptedToken = normalizeSettingsString(raw.gatewayTokenEncrypted);
    const plaintextToken = normalizeSettingsString(raw.gatewayToken);

    if (encryptedToken) {
      const decrypted = decryptToken(encryptedToken);
      if (decrypted) {
        settings.gatewayToken = decrypted;
      } else {
        console.warn(
          "[settings] Failed to decrypt gateway token — safeStorage may be unavailable",
        );
      }
    } else if (plaintextToken) {
      settings.gatewayToken = plaintextToken;

      // Migrate: encrypt and remove plaintext from disk
...
function writeDesktopSettings(settings: DesktopSettings): void {
  ...
  if (settings.gatewayHttpUrl) data.gatewayHttpUrl = settings.gatewayHttpUrl;
  if (settings.gatewayWsUrl) data.gatewayWsUrl = settings.gatewayWsUrl;
  if (settings.workspacePath) data.workspacePath = settings.workspacePath;
  if (settings.setupCompleted !== undefined)
    data.setupCompleted = settings.setupCompleted;
  data.updatedAt = new Date().toISOString();

  // Encrypt gateway token — never write plaintext to disk
  if (settings.gatewayToken) {
    const encrypted = encryptToken(settings.gatewayToken);
    if (encrypted) {
      data.gatewayTokenEncrypted = encrypted;
    } else {
      // safeStorage unavailable — warn but write plaintext as fallback
      console.error(
        "[settings] safeStorage unavailable — gateway token stored in plaintext",
      );
      data.gatewayToken = settings.gatewayToken;
    }
  }
...
async function spawnServer(): Promise<ChildProcess> {
  if (app.isPackaged) {
    ...
    if (!entry) {
      throw new Error(
        "Packaged server not found (expected server bundle under resources/server)",
      );
    }
    ...
    const workspaceRoot =
      settings.workspacePath || path.join(userDataDir, "workspace");
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      CLAWCONTROL_OPERATOR_AUTH_SECRET: ensureAuthSecret(),
      PATH: serverPath,
    };
  ...
  const proc = spawn("npm", ["--prefix", clawcontrolDir, "run", script], {
    ...
    env: {
      ...
      ...(settings.gatewayHttpUrl
        ? { OPENCLAW_GATEWAY_HTTP_URL: settings.gatewayHttpUrl }
        : {}),
      ...(settings.gatewayWsUrl
        ? { OPENCLAW_GATEWAY_WS_URL: settings.gatewayWsUrl }
        : {}),
      ...(settings.gatewayToken
        ? { OPENCLAW_GATEWAY_TOKEN: settings.gatewayToken }
        : {}),
    },
  });
...
async function restartManagedServer(): Promise<ServerRestartResponse> {
  ...
  if (!didSpawnServer || !serverProcess?.pid) {
    return {
      ok: false,
      message:
        "Saved configuration, but this server is externally managed. Restart it manually to apply workspace changes.",
    };
  }
  ...
  if (!stopped) {
    return {
      ok: false,
      message:
        "Saved configuration, but timed out while stopping the server. Restart manually.",
    };
  }
...
ipcMain.handle(
  RESTART_SERVER_CHANNEL,
  async (): Promise<ServerRestartResponse> => {
    try {
...
ipcMain.handle(
  ...
  ): Promise<RunModelAuthLoginResponse> => {
...
ipcMain.handle(
  ...
  async (): Promise<DesktopUpdateInfo> => {
    return getDesktopUpdateInfo();
  },
);

ipcMain.handle(
  GET_WHATS_NEW_CHANNEL,
  async (): Promise<WhatsNewPayload | null> => {
    return pendingWhatsNew;
  },
);

ipcMain.handle(
  ACK_WHATS_NEW_CHANNEL,
  async (_event, payload: { version?: unknown }): Promise<{ ok: boolean }> => {
    const currentVersion = app.getVersion();
    const requested =
      typeof payload?.version === "string"
        ? normalizeVersionInput(payload.version)
        : null;
  ...
  async (
    ...
    try {
      ...
      return {
        ok: false,
        message:
          err instanceof Error ? err.message : "Failed to open external URL.",
      };
    }
  },
);
